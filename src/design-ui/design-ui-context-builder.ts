...
export const BuildDesignUiContextInputSchema = z
  .object({
    ...
    designContractContent: z.string().trim().min(1).optional(),
    figmaInventoryContent: z.string().trim().min(1).optional(),
  })
  .strict();
...
Normalize Figma style-like component props into existing UI-library variants or tokens; record a design gap before using explicit style overrides.
Use component contracts as node-level acceptance criteria for dimensions, padding, radius, typography, icon/logo placement, and variant props. Full-screen visual similarity does not replace component-level evidence.
...
- [ ] typography
- [ ] icons/assets
- [ ] variant/token mapping for Figma style-like component props
- [ ] component contract metrics and variant isolation
