...
const PROMPT_INJECTION_LIKE_PATTERNS = [
  ...
  /reveal\s+.*secret/i,
  /exfiltrate/i,
  /\b(reveal|print|show|output|expose|leak|exfiltrate|disclose|dump)\b[\s\S]{0,80}\b(api\s*key|access\s*token|secret|credential)s?\b/i,
  /\b(api\s*key|access\s*token|secret|credential)s?\b[\s\S]{0,80}\b(reveal|print|show|output|expose|leak|exfiltrate|disclose|dump)\b/i,
  /이전\s*지시.*무시/,
  /시스템\s*프롬프트/,
  /개발자\s*메시지/,
  /비밀.*출력/,
  /토큰.*출력/,
  /API\s*키.*출력/,
  /(출력|노출|유출|공개|보여|표시|덤프)[\s\S]{0,80}(API\s*키|api\s*key|액세스\s*토큰|access\s*token|비밀|시크릿|토큰)/i,
  /(API\s*키|api\s*key|액세스\s*토큰|access\s*token|비밀|시크릿|토큰)[\s\S]{0,80}(출력|노출|유출|공개|보여|표시|덤프)/i,
  /모든\s*도구.*실행/,
] as const;
