...
export class SpecBddAgentLaneService {
  ...
  public async recordResult(rawInput: unknown) {
    ...
    const matrix = await readTestMatrix({
      run,
      artifactStore: this.artifactStore,
      relativePath: contextPack.gherkin.testMatrixPath,
    });
    const skeletons =
      input.writeToProject === true
        ? await writeAcceptanceSkeletons({
            projectRoot: run.projectRoot,
            changeName: input.changeName,
            matrix,
            force: input.force,
          })
        : { directory: `artifact-store://acceptance-skeletons/${input.changeName}`, files: [] };
    ...
    const report = SpecBddReviewReportSchema.parse({
      ...
      acceptanceSkeletonCount:
        input.writeToProject === true ? skeletons.files.length : input.acceptanceSkeletonCount,
      findings: input.findings,
      artifactIds: [reportJsonArtifactId, reportMdArtifactId],
    });
    const reportJson = `${JSON.stringify(report, null, 2)}\n`;
    const reportMd = renderSpecBddReviewMarkdown(report);
    const reportJsonArtifactPath = toRepoRelative(run.projectRoot, reportPaths.reportJsonPath);
    const reportMdArtifactPath = toRepoRelative(run.projectRoot, reportPaths.reportMdPath);

    if (input.writeToProject === true) {
    ...
    const reportJsonBlob =
      input.writeToProject === true
        ? undefined
        : await this.artifactStore.writeBlob({
            content: Buffer.from(reportJson, "utf8"),
            mediaType: "application/json",
            storedAt: timestamp,
            label: reportJsonArtifactPath,
          });
    const reportMdBlob =
      input.writeToProject === true
        ? undefined
    ...
    const reportJsonArtifact = ArtifactRefSchema.parse({
      ...
      metadata: {
        ...
        artifactPath: reportJsonArtifactPath,
        relativePath: reportJsonArtifactPath,
        storage: input.writeToProject === true ? "repo" : "artifact-store",
      },
    });
    const reportMdArtifact = ArtifactRefSchema.parse({
      ...
      metadata: {
        ...
        artifactPath: reportMdArtifactPath,
        relativePath: reportMdArtifactPath,
        storage: input.writeToProject === true ? "repo" : "artifact-store",
      },
    });
    ...
    const changedFiles =
      input.writeToProject === true
        ? unique([reportJsonArtifactPath, reportMdArtifactPath, ...skeletons.files])
        : [];
    ...
    return RecordSpecBddAgentResultSchema.parse({
      ...
      reportJsonPath: reportJsonArtifactPath,
      reportMarkdownPath: reportMdArtifactPath,
...
async function readTestMatrix(input: {
  run: {
    projectRoot: string;
  ...
  const artifact = input.run.artifacts.find(
    (candidate) =>
      candidate.kind === "test-matrix" &&
      (candidate.metadata["artifactPath"] === input.relativePath ||
        candidate.metadata["relativePath"] === input.relativePath),
  );

  if (artifact !== undefined) {
    const content = await input.artifactStore.readContent(artifact.digest as never);

    return TestMatrixSchema.parse(JSON.parse(content.toString("utf8")));
  }

  const absolutePath = path.join(input.run.projectRoot, input.relativePath);
  assertInsideProjectRoot(input.run.projectRoot, absolutePath);
