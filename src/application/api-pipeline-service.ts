...
export class ApiPipelineService {
  ...
  public async generate(rawInput: unknown) {
    ...
    const plan = await discoverApiGenerator({
      ...
      sourceKey: input.sourceKey,
      ...(input.targetWorkspace === undefined ? {} : { targetWorkspace: input.targetWorkspace }),
    ...
    const mock = generateMswHandlers({
      sourceKey: input.sourceKey,
      operations: inventory.operations,
      outputPath: `${apiSharedRoot(input.targetWorkspace)}/mocks/${input.sourceKey}.handlers.ts`,
    });
    ...
    const contractTest = generateContractTestSkeleton({
      sourceKey: input.sourceKey,
      operations: inventory.operations,
      outputPath: `${apiSharedRoot(input.targetWorkspace)}/__tests__/${input.sourceKey}.contract.generated.test.ts`,
    });
    ...
    const uiRoot = uiSourceRoot(input.targetWorkspace);
    const uiGlobs = [
      `${uiRoot}/pages/**/*.{ts,tsx}`,
      `${uiRoot}/widgets/**/*.{ts,tsx}`,
      `${uiRoot}/features/**/*.{ts,tsx}`,
    ];
    const sourceGuard = generateSourceGuardTest({
      generatedImportPattern: `${apiSharedRoot(input.targetWorkspace)}/generated/${input.sourceKey}`,
      uiGlobs,
      outputPath: `${apiSharedRoot(input.targetWorkspace)}/__tests__/source-guards.generated.test.ts`,
    });
...
function apiSharedRoot(targetWorkspace: string | undefined): string {
  return joinWorkspacePath(targetWorkspace, "src/shared/api");
}

function uiSourceRoot(targetWorkspace: string | undefined): string {
  return joinWorkspacePath(targetWorkspace, "src");
}
