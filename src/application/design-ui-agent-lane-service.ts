...
export class DesignUiAgentLaneService {
  ...
  public async prepare(rawInput: unknown) {
    ...
    const context = await buildDesignUiContextPack({
      ...
      ...(await optionalArtifactContent({
        artifactStore,
        artifact: findArtifact(run.artifacts, designContractArtifactId),
        key: "designContractContent",
      })),
      ...(figmaInventoryArtifactId === undefined
        ? {}
        : await optionalArtifactContent({
            artifactStore,
            artifact: findArtifact(run.artifacts, figmaInventoryArtifactId),
            key: "figmaInventoryContent",
          })),
    });
...
function findArtifact(artifacts: ArtifactRef[], artifactId: string): ArtifactRef | undefined {
  return artifacts.find((artifact) => artifact.id === artifactId);
}

async function optionalArtifactContent<
  T extends "designContractContent" | "figmaInventoryContent",
>(input: {
  ...
  try {
    const content = await input.artifactStore.readContent(input.artifact.digest);
    const text = content.toString("utf8");

    return text.trim().length === 0 ? {} : ({ [input.key]: text } as Record<T, string>);
  } catch {
    return {};
  }
}
