...
export const RecordLegacyVisualComparisonInputSchema = z
  .object({
    runId: RunIdSchema,
    changeName: z.string().trim().min(1),
    stateName: z.string().trim().min(1).default("legacy-vs-target"),
    legacyBaselineArtifactId: ArtifactIdSchema,
    targetScreenshotArtifactId: ArtifactIdSchema,
    diffArtifactId: ArtifactIdSchema.optional(),
    exactMatch: z.number().min(0).max(100),
    reviewMatch: z.number().min(0).max(100),
    threshold: z.number().min(0).max(100).optional(),
    decision: z.enum(["passed", "failed", "review-needed"]),
    notes: z.array(z.string().trim().min(1)).default([]),
  })
  .strict();

export const RecordLegacyVisualComparisonResultSchema = z
  .object({
    run: RunSummarySchema,
    report: VisualReportSchema,
    reportArtifactId: ArtifactIdSchema,
...
export class VisualRegressionService {
  ...
  public async compare(rawInput: unknown) {
    ...
    const report = VisualReportSchema.parse({
      runId: run.id,
      changeName: input.changeName,
      comparisonScope: visualReportScope(input.targets),
      generatedAt: timestamp,
    ...
    const reportJsonArtifact = await writeVisualBlob({
      ...
      metadata: {
        ...
        ...(report.comparisonScope === "component"
          ? {
              componentContractIds: input.targets
                .flatMap((target) => target.componentContractId ?? [])
                .join(","),
            }
          : {}),
        ...(input.targets[0]?.componentName === undefined
          ? {}
          : { componentName: input.targets[0].componentName }),
      },
    });
  ...
  public async recordLegacyComparison(rawInput: unknown) {
    const input = RecordLegacyVisualComparisonInputSchema.parse(rawInput);
    const run = await this.runStore.get(input.runId);
    const timestamp = IsoDateTimeSchema.parse(this.now());
    const baselineArtifact = requireArtifact(run.artifacts, input.legacyBaselineArtifactId);
    const targetArtifact = requireArtifact(run.artifacts, input.targetScreenshotArtifactId);
    const diffArtifact =
      input.diffArtifactId === undefined
        ? undefined
        : requireArtifact(run.artifacts, input.diffArtifactId);
    const dimensions = imageDimensions(
      await this.artifactStore.readContent(baselineArtifact.digest),
    );
    const exactMatchRatio = ratioFromInput(input.exactMatch);
    const reviewMatchRatio = ratioFromInput(input.reviewMatch);
    const gap =
      input.decision === "passed"
        ? undefined
        : createLegacyVisualGap({
            stateName: input.stateName,
            baselineArtifact,
            targetArtifact,
            timestamp,
            observed: `Review match was ${(reviewMatchRatio * 100).toFixed(2)}%.`,
            severity: input.decision === "failed" ? "major" : "minor",
          });
    const gapIds = gap === undefined ? [] : [gap.id];
    const report = VisualReportSchema.parse({
      runId: run.id,
      changeName: input.changeName,
      ...
      generatedAt: timestamp,
      targetCount: 1,
      passedCount: input.decision === "passed" ? 1 : 0,
      failedCount: input.decision === "failed" ? 1 : 0,
      reviewNeededCount: input.decision === "review-needed" ? 1 : 0,
      results: [
        {
          targetId: input.stateName,
          status: input.decision,
          figmaScreenshotArtifactId: baselineArtifact.id,
          browserScreenshotArtifactId: targetArtifact.id,
          ...(diffArtifact === undefined ? {} : { diffArtifactId: diffArtifact.id }),
          metrics: {
            width: dimensions.width,
            height: dimensions.height,
            comparedPixelCount: dimensions.width * dimensions.height,
            maskedPixelCount: 0,
            exactMatchRatio,
            reviewMatchRatio,
            meanDistance: 0,
    ...
    const reportJsonArtifact = await writeVisualBlob({
      ...
      content: Buffer.from(`${JSON.stringify(report, null, 2)}\n`, "utf8"),
      mediaType: "application/json",
      label: "legacy-visual-report-json",
      generatedAt: timestamp,
      kind: "visual-report",
      metadata: {
        adapter: "legacy-vs-target-visual",
        reportKind: "visual-report-json",
        ...
        changeName: input.changeName,
        stateName: input.stateName,
        legacyBaselineArtifactId: baselineArtifact.id,
        targetScreenshotArtifactId: targetArtifact.id,
        ...(diffArtifact === undefined ? {} : { diffArtifactId: diffArtifact.id }),
        exactMatch: exactMatchRatio,
        reviewMatch: reviewMatchRatio,
        ...(input.threshold === undefined ? {} : { threshold: ratioFromInput(input.threshold) }),
    ...
    const nextRun = RunManifestSchema.parse({
      ...
      gaps: gap === undefined ? run.gaps : [...run.gaps, gap],
    });

    await this.runStore.save(nextRun, run.revision);

    return RecordLegacyVisualComparisonResultSchema.parse({
      run: summarizeRun(nextRun),
      report,
      reportArtifactId: reportJsonArtifact.id,
      markdownReportArtifactId: reportMdArtifact.id,
...
function targetFromFigmaScreenshot(input: {
  ...
  return VisualTargetSchema.parse({
    ...
    comparisonScope:
      input.artifact.metadata["comparisonScope"] === "component" ? "component" : "screen",
    ...optionalMetadataString(
      input.artifact.metadata["componentContractId"],
      "componentContractId",
    ),
    ...optionalMetadataString(input.artifact.metadata["componentName"], "componentName"),
    masks: parseMasks(input.artifact.metadata["masks"]),
    status: "planned",
  });
}

function visualReportScope(targets: VisualTarget[]): "screen" | "component" {
  return targets.length > 0 && targets.every((target) => target.comparisonScope === "component")
    ? "component"
    : "screen";
}
...
function createLegacyVisualGap(input: {
  stateName: string;
  ...
  return GapSchema.parse({
    ...
    severity: input.severity,
    status: "open",
    title: `Legacy visual mismatch for ${input.stateName}`,
    expected: `Target screenshot should match legacy baseline ${input.baselineArtifact.id}.`,
    observed: input.observed,
    ...
    metadata: {
      comparisonMode: "legacy-vs-target",
      legacyBaselineArtifactId: input.baselineArtifact.id,
      targetScreenshotArtifactId: input.targetArtifact.id,
    },
  });
}

function imageDimensions(content: Buffer): { width: number; height: number } {
...
function ratioFromInput(value: number): number {
