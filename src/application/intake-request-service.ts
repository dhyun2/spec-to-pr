...
function extractFilePaths(text: string): string[] {
  ...
  const fileMatches = text.matchAll(
    /(?<![\p{L}\p{N}_@./-])((?:\.{1,2}\/)?[\p{L}\p{N}_@./-]+\.(?:md|mdx|txt|yaml|yml|json|jsonc|feature|pdf|html|htm))(?![A-Za-z0-9_@./-])/gu,
  );
  ...
  const directoryMatches = text.matchAll(
    /(?<![\p{L}\p{N}_@./-])((?:\.{1,2}\/)?[\p{L}\p{N}_@.-]+(?:\/[\p{L}\p{N}_@.-]+)+)(?![\p{L}\p{N}_@./-])/gu,
  );
...
function addPath(paths: Set<string>, rawPath: string | undefined): void {
  ...
  const normalized = stripKoreanPathParticle(rawPath.replace(/^\.\//, ""));
...
function stripKoreanPathParticle(pathLike: string): string {
  const trimmed = trimTrailingPunctuation(pathLike);
  ...
  const strippedLastSegment = lastSegment.replace(
    /(이고요?|에서|으로|로|에는|에게|까지|부터|은|는|을|를|이|가|도|랑|와|과|의)$/u,
    "",
  );

  return [...segments.slice(0, -1), strippedLastSegment].join("/");
}
...
function buildMinimalOpenApiDocument(endpoints: EndpointNote[]): Record<string, unknown> {
  ...
  for (const endpoint of endpoints) {
    ...
    paths[endpoint.path]![endpoint.method.toLowerCase()] = {
      operationId: endpoint.operationId,
      summary: `${endpoint.method} ${endpoint.path} endpoint noted in the user request. Request and response schemas are unconfirmed.`,
      "x-spec-to-pr-source-role": "endpoint-inventory-only",
      responses: {
        "200": {
          description:
            "Endpoint inventory placeholder only. Confirm request and response schemas from API docs before treating this as a contract.",
        },
      },
    };
  }
...
function extractPublishPolicy(text: string): z.infer<typeof PublishPolicySchema> {
  ...
  const publishDenied = hasPublishDeniedIntent(text);
  const shouldPublish = publishDenied ? false : hasPublishCreateIntent(text) ? true : undefined;
  const mergeAllowed =
    publishDenied ||
...
function hasPublishCreateIntent(text: string): boolean {
  return [
    /\b(?:publish|open|create|raise|submit)\b[^\n]{0,80}\b(?:pr|mr|pull\s*request|merge\s*request)\b/i,
    /\b(?:pr|mr|pull\s*request|merge\s*request)\b[^\n]{0,80}\b(?:publish|open|create|raise|submit)\b/i,
    /\b(?:PR|MR|pull\s*request|merge\s*request)\b[^\n]{0,80}(?:올려|올려도|생성|열어|요청|publish)/i,
    /(?:올려|올려도|생성|열어|publish)[^\n]{0,80}\b(?:PR|MR|pull\s*request|merge\s*request)\b/i,
  ].some((pattern) => pattern.test(text));
}

function hasPublishDeniedIntent(text: string): boolean {
  return [
    /\b(?:pr|mr|pull\s*request|merge\s*request)\b[^\n]{0,80}\b(?:do\s*not|don't|never|not\s*yet)\b[^\n]{0,80}\b(?:publish|open|create|raise|submit)\b/i,
    /\b(?:do\s*not|don't|never|not\s*yet)\b[^\n]{0,80}\b(?:publish|open|create|raise|submit)\b[^\n]{0,80}\b(?:pr|mr|pull\s*request|merge\s*request)\b/i,
    /\b(?:PR|MR|pull\s*request|merge\s*request)\b[^\n]{0,80}(?:아직[^\n]{0,30})?(?:올리지\s*마|올리지마|생성하지\s*마|열지\s*마|publish\s*하지\s*마)/i,
    /(?:올리지\s*마|올리지마|생성하지\s*마|열지\s*마|publish\s*하지\s*마)[^\n]{0,80}\b(?:PR|MR|pull\s*request|merge\s*request)\b/i,
  ].some((pattern) => pattern.test(text));
}
