...
export class QualityGateService {
  ...
  public async run(rawInput: unknown) {
    ...
    const report = QualityGateReportSchema.parse({
      ...
      status: failedChecks.length === 0 && skippedChecks.length === 0 ? "passed" : "failed",
      startedAt,
      completedAt,
    ...
    const gaps = [
      ...failedChecks.map((check) => gapFromFailedCheck(check, completedAt)),
      ...skippedChecks.map((check) => gapFromSkippedCheck(check, completedAt)),
    ];
    const existingGaps = resolveSupersededQualityGateGaps({
      gaps: run.gaps,
      checks,
...
function gapFromSkippedCheck(check: CheckResult, timestamp: string): Gap {
  return GapSchema.parse({
    id: createGapId(),
    category: ["unit", "component", "contract", "acceptance"].includes(check.kind)
      ? "test"
    ...
    severity: ["typecheck", "build", "contract", "acceptance"].includes(check.kind)
      ...
      : "major",
    status: "open",
    title: `Quality gate skipped: ${check.name}`,
    expected: `${check.name} quality gate should run or be explicitly overridden.`,
    observed: check.skipReason ?? check.summary,
    impact: "Skipped quality gates leave required verification incomplete.",
    sourceEvidenceIds: [],
    owner: "implementation",
    createdAt: timestamp,
    updatedAt: timestamp,
...
function resolveSupersededQualityGateGaps(input: {
  gaps: Gap[];
  checks: CheckResult[];
  reportArtifactId: string;
  timestamp: string;
  ...
  return input.gaps.map((gap) => {
    ...
    const matchingPassedCheck = passedChecks.find((check) =>
      qualityGateGapMatchesCheck(gap, check),
    );

    if (matchingPassedCheck === undefined) {
      return gap;
    }
    ...
    return GapSchema.parse({
      ...
      resolutionArtifactIds: [input.reportArtifactId],
      metadata: {
        ...gap.metadata,
        supersededByCheckId: matchingPassedCheck.id,
      },
    });
  });
}
...
function qualityGateGapMatchesCheck(gap: Gap, check: CheckResult): boolean {
  ...
  return checkName === undefined || checkName === check.name;
}
