...
export const GeneratePrReportResultSchema = z
  .object({
    ...
    auditMarkdownArtifactId: ArtifactIdSchema,
...
export class PrReportService {
  ...
  public async generatePrReport(rawInput: unknown) {
    ...
    const auditMarkdownArtifactId = createArtifactId();
    const viewModelArtifactId = createArtifactId();
    const viewModel = PrReportViewModelSchema.parse({
      ...collectedViewModel,
      reportArtifactIds: [markdownArtifactId, auditMarkdownArtifactId, viewModelArtifactId],
    });
    const finalMarkdown = renderPrBodyMarkdown(viewModel);
    const auditMarkdown = renderPrReportMarkdown(viewModel);
    ...
    const nextRun = RunManifestSchema.parse({
      ...
      artifacts: [
        ...run.artifacts,
        finalWritten.markdownArtifact,
        finalWritten.auditMarkdownArtifact,
        finalWritten.viewModelArtifact,
      ],
    });
    ...
    return GeneratePrReportResultSchema.parse({
      ...
      auditMarkdownArtifactId: finalWritten.auditMarkdownArtifact.id,
