...
export const OpenApiGapCandidateCodeSchema = z.enum([
  ...
  "endpoint-inventory-schema-missing",
...
export function detectOpenApiGapCandidates(input: {
  ...
  gaps.push(...detectEndpointInventoryOnlyGaps(input.parsed, input.inventory.operations));
...
function detectEndpointInventoryOnlyGaps(
  parsed: ParsedOpenApiDocument,
  operations: OpenApiOperationInventoryItem[],
): OpenApiGapCandidate[] {
  const rootRole = parsed.document["x-spec-to-pr-source-role"];

  if (rootRole !== "endpoint-inventory-only") {
    return [];
  }

  return operations.map((operation) =>
    gap({
      code: "endpoint-inventory-schema-missing",
      severity: "major",
      category: "api",
      title: "Endpoint note lacks request/response schema",
      expected:
        "Endpoint notes should be confirmed by OpenAPI request and response schemas before client or wrapper generation treats them as contracts.",
      observed: `${operation.method.toUpperCase()} ${operation.path} came from endpoint notes only; no request or response schema was provided.`,
      impact:
        "Generated API clients, wrappers, mocks, and tests would rely on assumptions instead of API documentation.",
