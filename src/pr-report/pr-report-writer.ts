...
export type PrReportWriteResult = {
  ...
  auditMarkdownArtifact: ArtifactRef;
...
export async function writePrReportArtifacts(input: {
  ...
  auditMarkdownArtifactId?: string | undefined;
  ...
  const auditMarkdownArtifact = await writeArtifact({
    artifactStore: input.artifactStore,
    artifactId: input.auditMarkdownArtifactId,
    label: "pr-audit-report.md",
    content: input.auditMarkdown,
    mediaType: "text/markdown",
    generatedAt: input.generatedAt,
    metadata: {
      reportKind: "pr-audit-markdown",
      decision: input.viewModel.decision,
      locale: input.viewModel.locale,
  ...
  const viewModelArtifact = await writeArtifact({
    ...
    metadata: {
      ...
      locale: input.viewModel.locale,
      markdownArtifactId: markdownArtifact.id,
      auditMarkdownArtifactId: auditMarkdownArtifact.id,
    },
  });

  return {
    markdownArtifact,
    auditMarkdownArtifact,
    viewModelArtifact,
    viewModelJson,
  };
}
