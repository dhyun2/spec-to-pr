...
import { latestReviewScorecard, scorecardGateNote } from "../review-scorecard/index.js";
...
import {
  activeGapsForLatestChecks,
  buildReportGateRequirements,
  decideReportStatus,
  selectLatestChecksByKind,
} from "./pr-report-decision-policy.js";
import {
...
import {
  ...
  isLegacyVsTargetVisualComparisonArtifact,
  isVisualComparisonArtifact,
  visualComparisonStatusFromMetadata,
  visualPercentFromMetadata,
...
export function collectPrReportViewModel(input: {
  ...
  const allChecks = selectLatestChecksByKind(
    input.run.agentResults.flatMap((result) => result.checks),
  );
  const activeGaps = activeGapsForLatestChecks(input.run.gaps, allChecks);
  const openGaps = activeGaps.filter((gap) => gap.status === "open" || gap.status === "assumed");
  ...
  return PrReportViewModelSchema.parse({
    ...
    scorecardRows: scorecardRows(input.run.artifacts),
    specificationLinks: specificationLinks(input.run.artifacts),
    traceabilityRows: [],
    traceabilityRowCount: traceabilityRowCount(input.run.artifacts),
...
function traceabilityRowCount(artifacts: ArtifactRef[]): number {
  return artifacts
    .filter((artifact) => artifact.kind === "traceability-matrix")
    .reduce((maximum, artifact) => {
      const rowCount = artifact.metadata["rowCount"];

      return typeof rowCount === "number" && Number.isFinite(rowCount)
        ? Math.max(maximum, rowCount)
        : maximum;
    }, 0);
}
...
function titleForRun(run: RunManifest, locale: ReportLocale): string {
  ...
  return locale === "ko" ? `SpecToPR 리포트 - ${changeName}` : `SpecToPR Report - ${changeName}`;
}
...
function gateRows(
  ...
  return [
    ...
    },
    {
      ...
      required: true,
      ...reviewScorecardGateStatus(artifacts),
    },
    {
    ...
    {
      gate: "Component visual regression",
      required: hasComponentContracts(artifacts),
      ...componentVisualGateStatus({ artifacts }),
    },
    {
      gate: "Accessibility",
...
function reviewScorecardGateStatus(
  artifacts: ArtifactRef[],
  ...
  if (scorecard === undefined) {
    return {
      status: "not-run",
      evidence: [],
      notes: "No review scorecard artifact was recorded.",
    };
  }

  return {
    status: scorecard.dimensions.some((dimension) => dimension.status === "fail")
      ? "fail"
      : scorecard.dimensions.some((dimension) => dimension.status === "warning")
        ? "warning"
        : "pass",
    evidence: [scorecard.artifactId],
    notes: scorecardGateNote(scorecard),
  };
}

function scorecardRows(artifacts: ArtifactRef[]) {
  const scorecard = latestReviewScorecard(artifacts);
  ...
  return scorecard.dimensions.map((dimension) => ({
    id: dimension.id,
    ...
    nextRepairTarget: dimension.nextRepairTarget,
  }));
}
...
function figmaInventoryRows(artifacts: ArtifactRef[]): ReportArtifactSummaryRow[] {
  return [
    ...
    componentContractSummaryRow(artifacts),
  ];
}
...
function visualGateStatus(input: {
  ...
  const visualReports = input.artifacts.filter((artifact) => isVisualComparisonArtifact(artifact));
  const visualCheckStatus = statusForCheckKinds(input.checks, ["visual"], "visual");

  if (visualReports.length > 0) {
    const reportStatuses = visualReports.map((artifact) =>
      visualComparisonStatusFromMetadata(artifact.metadata["decision"], "pass"),
    );
    const hasLegacyVsTarget = visualReports.some(isLegacyVsTargetVisualComparisonArtifact);
    const status =
      visualCheckStatus.status === "fail" || reportStatuses.includes("fail")
        ? "fail"
        : reportStatuses.includes("warning")
          ? "warning"
          : "pass";
    ...
    return {
      ...
      notes: hasLegacyVsTarget
        ? "legacy-vs-target visual comparison artifact is recorded."
        : "Figma/browser visual comparison artifact is recorded.",
    };
  }
...
function componentVisualGateStatus(input: {
  artifacts: ArtifactRef[];
}): Pick<ReportGateRow, "status" | "evidence" | "notes"> {
  if (!hasComponentContracts(input.artifacts)) {
    return {
      status: "not-applicable",
      evidence: [],
      notes: "No component contracts were recorded for this run.",
    };
  }

  const componentReports = input.artifacts.filter(isComponentVisualComparisonArtifact);

  if (componentReports.length === 0) {
    return {
      status: "not-run",
      evidence: [],
      notes:
        "Component contracts exist, but no component-level visual comparison artifact was recorded.",
    };
  }

  const statuses = componentReports.map((artifact) =>
    visualComparisonStatusFromMetadata(artifact.metadata["decision"], "pass"),
  );
  ...
  return {
    status,
    evidence: componentReports.map((artifact) => artifact.id),
    notes: "Component-level visual comparison artifact is recorded.",
  };
}
...
function componentContractSummaryRow(artifacts: ArtifactRef[]): ReportArtifactSummaryRow {
  ...
  if (matchingArtifacts.length === 0) {
    return {
      item: "Component contracts",
      status: "not-run",
      artifacts: [],
      notes: "No component contract artifact was recorded.",
    };
  }
...
function visualRows(artifacts: ArtifactRef[], checks: CheckResult[]) {
  const visualReports = artifacts.filter((artifact) => isVisualComparisonArtifact(artifact));
  ...
  return visualReports.map((artifact) => ({
    state: String(
      isComponentVisualComparisonArtifact(artifact)
        ? (artifact.metadata["componentName"] ??
            artifact.metadata["componentContractId"] ??
            artifact.id)
        : (artifact.metadata["stateName"] ?? artifact.metadata["changeName"] ?? artifact.id),
    ),
    ...visualArtifactIds(artifact),
    exactMatch: visualPercentFromMetadata(
      artifact.metadata["exactMatch"] ?? artifact.metadata["exactMatchRatio"],
    ),
    reviewMatch: visualPercentFromMetadata(
      artifact.metadata["reviewMatch"] ?? artifact.metadata["reviewMatchRatio"],
    ),
    result: visualComparisonStatusFromMetadata(artifact.metadata["decision"], "warning"),
    notes: visualNotesForArtifact(artifact),
  }));
}

function visualArtifactIds(artifact: ArtifactRef) {
  if (isLegacyVsTargetVisualComparisonArtifact(artifact)) {
    return {
      ...optionalArtifactId(artifact.metadata["legacyBaselineArtifactId"], "figmaArtifactId"),
      ...optionalArtifactId(artifact.metadata["targetScreenshotArtifactId"], "browserArtifactId"),
      ...optionalArtifactId(artifact.metadata["diffArtifactId"], "diffArtifactId"),
    };
  }

  return {
    ...optionalArtifactId(artifact.metadata["figmaScreenshotArtifactId"], "figmaArtifactId"),
    ...optionalArtifactId(artifact.metadata["browserScreenshotArtifactId"], "browserArtifactId"),
    ...optionalArtifactId(artifact.metadata["diffArtifactId"], "diffArtifactId"),
  };
}

function optionalArtifactId(value: unknown, key: string): Record<string, string> {
  return typeof value === "string" && value.trim().length > 0 ? { [key]: value } : {};
}

function visualNotesForArtifact(artifact: ArtifactRef): string {
  if (isComponentVisualComparisonArtifact(artifact)) {
    return "component-level visual comparison artifact is available. Review crop bounds, contract metrics, variant props, threshold, fixture data, and CSS leakage risk in the artifact.";
  }

  if (isLegacyVsTargetVisualComparisonArtifact(artifact)) {
    return "legacy-vs-target visual comparison artifact is available. Review thresholds, viewport, browser, fixture data, and tolerated dynamic regions in the artifact.";
  }

  return "Visual report artifact is available. Review algorithm, thresholds, masks, viewport, browser, and font environment in the artifact.";
}
