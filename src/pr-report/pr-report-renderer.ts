...
export function renderPrBodyMarkdown(model: PrReportViewModel): string {
  if (model.locale === "ko") {
    return renderKoreanPrBodyMarkdown(model);
  }

  return renderEnglishPrBodyMarkdown(model);
}

function renderEnglishPrBodyMarkdown(model: PrReportViewModel): string {
  return `${[
    renderSummary(model),
    renderDecision(model, { compact: true }),
    renderPriorityFindings(model),
    renderGateSummary(model, { compactEvidence: true }),
    renderReviewScorecard(model),
    renderVisual(model),
    renderFunctional(model),
    renderApi(model),
    renderAccessibility(model),
    renderPerformance(model),
    renderGroupedGaps(model),
  ]
    .join("\n\n")
    .trimEnd()}\n`;
}

function renderKoreanPrBodyMarkdown(model: PrReportViewModel): string {
  return `${[
    renderKoreanSummary(model),
    renderKoreanDecision(model, { compact: true }),
    renderKoreanPriorityFindings(model),
    renderKoreanGateSummary(model, { compactEvidence: true }),
    renderKoreanReviewScorecard(model),
    ...
    rows.length === 0
      ...
      : markdownTable(
          ...
          rows.map((row) => [
            ...
            row.required ? "Yes" : "No",
            row.status,
            evidenceCell(row.evidence, "en", options.compactEvidence ?? false),
            row.notes,
          ]),
        ),
  ].join("\n");
...
function renderReviewScorecard(model: PrReportViewModel): string {
  const rows = model.scorecardRows ?? [];

  return [
    "## Review Scorecard",
    "",
    rows.length === 0
      ? "No review scorecard rows were recorded. The end-to-end workflow must generate a scorecard before publishing."
      : markdownTable(
          ["Dimension", "Score", "Threshold", "Status", "Next", "Evidence", "Notes"],
          rows.map((row) => [
            row.label,
            row.score.toFixed(2),
            row.threshold.toFixed(2),
            row.status,
            row.nextRepairTarget ? "next repair target" : "-",
            evidenceCell(row.evidence ?? [], "en", true),
            row.notes,
          ]),
        ),
  ].join("\n");
...
function renderPriorityFindings(model: PrReportViewModel): string {
  const findings = priorityFindings(model, "en");

  return [
    "## Priority Failures / Blockers",
    "",
    findings.length === 0
      ? "No blocking or failed review findings were identified from the report model."
      : findings.map((finding) => `- ${finding}`).join("\n"),
  ].join("\n");
}

function renderKoreanPriorityFindings(model: PrReportViewModel): string {
  const findings = priorityFindings(model, "ko");
...
function priorityFindings(model: PrReportViewModel, locale: "ko" | "en"): string[] {
  const findings: string[] = [];

  for (const row of model.gateRows.filter(
    (gate) => gate.required && ["fail", "not-run", "skipped"].includes(gate.status),
  )) {
    findings.push(
      locale === "ko"
        ? `${koreanGateName(row.gate)}: ${koreanStatus(row.status)} - ${koreanNote(row.notes)}`
        : `${row.gate}: ${row.status} - ${row.notes}`,
    );
  }

  if (traceabilityRowCount(model) === 0) {
    findings.push(
      locale === "ko"
        ? "요구사항 추적성: traceability matrix row가 0개입니다. Build Traceability 단계가 실행되지 않았거나 brief/Figma/API evidence linking이 누락되었습니다."
        : "Requirement traceability: 0 traceability matrix rows were recorded. Build Traceability may not have run, or brief/Figma/API evidence linking is missing.",
    );
  }

  for (const gap of groupedGaps(model.gapSummaries).filter(
    (row) => row.severity === "blocker" && ["open", "assumed"].includes(row.status),
  )) {
    findings.push(
      locale === "ko"
        ? `${gap.title}: ${gap.count}개 ${gap.severity} gap`
        : `${gap.title}: ${gap.count} ${gap.severity} gap(s)`,
    );
  }
...
function renderRequirementTraceability(model: PrReportViewModel): string {
  return [
    ...
    model.traceabilityRows.length === 0
      ? traceabilityRowCount(model) > 0
        ? `${traceabilityRowCount(model)} traceability matrix row(s) were recorded in artifacts. Detailed rows are stored in the traceability matrix artifact.`
...
function renderGroupedGaps(model: PrReportViewModel): string {
  const rows = groupedGaps(model.gapSummaries);
  ...
  return [
    ...
    rows.length === 0
      ? "No open or assumed gaps were found."
      : markdownTable(
          ["Category", "Severity", "Status", "Title", "Count", "Impact"],
          rows.map((gap) => [
            gap.category,
            gap.severity,
            gap.status,
...
function renderKoreanGroupedGaps(model: PrReportViewModel): string {
  const rows = groupedGaps(model.gapSummaries);
...
function groupedGaps(gaps: PrReportViewModel["gapSummaries"]): GroupedGap[] {
  const groups = new Map<string, GroupedGap>();

  for (const gap of gaps) {
    const key = [gap.category, gap.severity, gap.status, gap.title, gap.impact].join("\u0000");
...
function renderDecision(model: PrReportViewModel, options: { compact?: boolean } = {}): string {
  ...
  return ["## Decision", "", markdownTable(["Item", "Result"], rows)].join("\n");
}
...
function renderKoreanGateSummary(
  model: PrReportViewModel,
  options: { compactEvidence?: boolean } = {},
): string {
  const rows = model.gateRows ?? [];
    ...
    rows.length === 0
      ...
      : markdownTable(
          ...
          rows.map((row) => [
            ...
            evidenceCell(row.evidence, "ko", options.compactEvidence ?? false),
...
function renderKoreanReviewScorecard(model: PrReportViewModel): string {
  const rows = model.scorecardRows ?? [];
  ...
  return [
    ...
    "",
    rows.length === 0
      ? "리뷰 점수표 행이 없습니다. end-to-end workflow는 publish 전에 scorecard를 생성해야 합니다."
      : markdownTable(
          ...
          rows.map((row) => [
            ...
            row.threshold.toFixed(2),
            koreanStatus(row.status),
            row.nextRepairTarget ? "다음 수정 대상" : "-",
            evidenceCell(row.evidence ?? [], "ko", true),
            koreanNote(row.notes),
          ]),
        ),
  ].join("\n");
...
function renderKoreanRequirementTraceability(model: PrReportViewModel): string {
  return [
    ...
    model.traceabilityRows.length === 0
      ? traceabilityRowCount(model) > 0
        ? `${traceabilityRowCount(model)}개 추적성 매트릭스 행이 artifact에 기록되어 있습니다. 상세 행은 traceability matrix artifact에 보관됩니다.`
        : "추적성 매트릭스 행이 없습니다."
      : markdownTable(
...
function evidenceCell(evidence: string[], locale: "ko" | "en", compact: boolean): string {
  ...
  if (locale === "ko") {
    return `${evidence.length}개 증거`;
  }

  return `${evidence.length} evidence item${evidence.length === 1 ? "" : "s"}`;
}

function traceabilityRowCount(model: PrReportViewModel): number {
  return Math.max(model.traceabilityRows.length, model.traceabilityRowCount ?? 0);
}
...
function renderKoreanDecision(
  ...
  return ["## 결정", "", markdownTable(["항목", "결과"], rows)].join("\n");
}
