...
export async function buildSpecBddContextPack(input: {
  ...
  await assertArtifactOrReadable({
    run: input.run,
    artifactPath: toRepoRelativePath(input.run.projectRoot, paths.manifestPath),
    filePath: paths.manifestPath,
  });
  await assertArtifactOrReadable({
    run: input.run,
    artifactPath: toRepoRelativePath(input.run.projectRoot, testMatrixPath),
  ...
  return SpecBddContextPackSchema.parse({
    ...
    allowedWritePaths: [],
    expectedOutputs: [
      `artifact-store://openspec/changes/${changeName}/artifacts/spec-bdd-review.md`,
      `artifact-store://openspec/changes/${changeName}/artifacts/spec-bdd-review.json`,
    ],
  });
}
...
async function assertArtifactOrReadable(input: {
  run: RunManifest;
  artifactPath: string;
  filePath: string;
}): Promise<void> {
  if (
    input.run.artifacts.some(
      (artifact) =>
        artifact.metadata["artifactPath"] === input.artifactPath ||
        artifact.metadata["relativePath"] === input.artifactPath,
    )
  ) {
    return;
  }
