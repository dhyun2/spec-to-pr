...
import {
  BuildFeatureCoverageMatrixInputSchema,
  BuildFeatureCoverageMatrixResultSchema,
  GenerateLegacyFeatureInventoryInputSchema,
  GenerateLegacyFeatureInventoryResultSchema,
} from "../application/legacy-coverage-service.js";
import {
...
import {
  ...
  RecordLegacyVisualComparisonInputSchema,
  RecordLegacyVisualComparisonResultSchema,
...
import {
  GenerateReviewScorecardInputSchema,
  GenerateReviewScorecardResultSchema,
} from "../application/review-scorecard-service.js";
import {
...
const TOOL_NAMES = [
  ...
  "record_legacy_visual_comparison",
  "create_run",
  "get_run",
  "list_runs",
  "start_stage",
  ...
  "generate_legacy_feature_inventory",
...
export function createKernelServer(servicesProvider: ServicesProvider): McpServer {
  ...
  server.registerTool(
    "record_legacy_visual_comparison",
    {
      title: "Record legacy visual comparison",
      description:
        "Record existing legacy baseline, target screenshot, and diff PNG artifacts as official visual-report-json evidence.",
      inputSchema: RecordLegacyVisualComparisonInputSchema.shape,
      outputSchema: RecordLegacyVisualComparisonResultSchema.shape,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
        idempotentHint: false,
    ...
    async (input: unknown) =>
      handleTool(async () => {
        const { visualRegressionService } = await servicesProvider();
        const structuredContent = await visualRegressionService.recordLegacyComparison(input);

        return {
          text: `Recorded legacy-vs-target visual comparison ${structuredContent.decision}.`,
          structuredContent,
        };
      }),
  );
  ...
  server.registerTool(
    "generate_legacy_feature_inventory",
    {
      title: "Generate legacy feature inventory",
      description:
        "Scan a legacy source root and record route, component, API, native bridge, event, URL, analytics, and branch behavior features for coverage review.",
      inputSchema: GenerateLegacyFeatureInventoryInputSchema.shape,
      outputSchema: GenerateLegacyFeatureInventoryResultSchema.shape,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
        idempotentHint: true,
    ...
    async (input: unknown) =>
      handleTool(async () => {
        const { legacyCoverageService } = await servicesProvider();
        const structuredContent = await legacyCoverageService.generateInventory(input);

        return {
          text: `Generated legacy feature inventory with ${structuredContent.featureCount} feature(s) from ${structuredContent.scannedFileCount} file(s).`,
          structuredContent,
        };
      }),
  );
  ...
  server.registerTool(
    "build_feature_coverage_matrix",
    {
      title: "Build feature coverage matrix",
      description:
        "Compare legacy feature inventory against OpenSpec, Gherkin, target implementation, and test evidence; record blocker gaps for uncovered features.",
      inputSchema: BuildFeatureCoverageMatrixInputSchema.shape,
      outputSchema: BuildFeatureCoverageMatrixResultSchema.shape,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
        idempotentHint: true,
    ...
    async (input: unknown) =>
      handleTool(async () => {
        const { legacyCoverageService } = await servicesProvider();
        const structuredContent = await legacyCoverageService.buildCoverageMatrix(input);

        return {
          text: `Built feature coverage matrix: ${structuredContent.coveredCount}/${structuredContent.featureCount} covered, ${structuredContent.uncoveredCount} uncovered.`,
          structuredContent,
        };
      }),
  );
  ...
  server.registerTool(
    "generate_review_scorecard",
    {
      title: "Generate review scorecard",
      description:
        "Score brief fidelity, legacy coverage, Gherkin completeness, TDD evidence, design-system usage, visual parity, resource contracts, API contracts, and publish synchronization.",
      inputSchema: GenerateReviewScorecardInputSchema.shape,
      outputSchema: GenerateReviewScorecardResultSchema.shape,
      annotations: {
        readOnlyHint: false,
        destructiveHint: false,
        idempotentHint: false,
    ...
    async (input: unknown) =>
      handleTool(async () => {
        const { reviewScorecardService } = await servicesProvider();
        const structuredContent = await reviewScorecardService.generate(input);

        return {
          text: `Review scorecard ${structuredContent.decision.status}: lowest score ${structuredContent.decision.score.toFixed(2)}.`,
          structuredContent,
        };
      }),
  );
