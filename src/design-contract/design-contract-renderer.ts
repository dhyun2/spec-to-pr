...
export function renderDesignContract(input: {
  ...
  return {
    ...
    componentContractsJson: `${JSON.stringify(input.contract.componentContracts, null, 2)}\n`,
...
function renderContractMarkdown(contract: FigmaDesignContract): string {
  return markdown([
    ...
    `- Component contracts: ${contract.componentContracts.length}`,
    ...
    "## Component Contracts",
    "",
    "| Figma Node | Component | Variant | Metrics | Min Visual Match | Gaps |",
    "|---|---|---|---|---|---|",
    ...contract.componentContracts.map((contract) =>
      [
        `${escapeCell(contract.figmaName)} (${contract.figmaNodeId})`,
        contract.codeComponent ?? "-",
        escapeCell(contract.variantKey),
        String(contract.metrics.length),
        `${Math.round(contract.acceptance.minReviewMatch * 100)}%`,
        contract.gapIds.join("<br>") || "-",
      ].join(" | "),
    ),
    "",
    "## Token Mappings",
...
function renderUiRules(contract: FigmaDesignContract): string {
  return markdown([
    ...
    "8. Normalize Figma style-like component props into supported UI-library variants or design tokens when an existing UI library or design system is available (for example size, intent, tone, state, density, radius, or elevation).",
    "9. Do not pass Figma style-like props through as ad-hoc code props or inline styles; if no variant or token exists, record a design gap before using an explicit style override.",
    "10. Treat `component-contracts.json` as acceptance criteria for node-level UI parts. Lock height, padding, radius, typography, icon/logo placement, button metrics, and variant props before relying on full-screen visual score.",
    "11. For each component contract, record component-level visual comparison evidence with `comparisonScope: component` and `reviewMatch` at or above the contract threshold before marking the design gate complete.",
    "",
    "## Agent Consumption",
    "",
