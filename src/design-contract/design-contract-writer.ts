...
export async function writeDesignContractArtifacts(input: {
  ...
  const files = [
    ...
    {
      name: "component-contracts.json",
      content: input.rendered.componentContractsJson,
      mediaType: "application/json",
      kind: "design-system-map" as const,
    },
    {
  ...
  const artifactRefs = written.map((file) =>
    ArtifactRefSchema.parse({
      ...
      metadata: {
        ...
        componentContractCount: componentContractCount(input.rendered.componentContractsJson),
      },
    }),
  );
...
function componentContractCount(content: string): number {
  try {
