...
export function buildFigmaDesignContract(input: {
  ...
  const componentContracts = inventory.components.map((component, index) =>
    buildComponentContract({
      component,
      mapping: componentMappings[index],
      evidence: input.evidence,
    }),
  );
...
function buildComponentContract(input: {
  component: Record<string, unknown>;
  ...
  const figmaName = readName(input.component);
  const figmaNodeId = readNodeId(input.component);
  const variantProperties = readStringRecord(input.component, "variantProperties");
  const codeConnect = readRecord(input.component, "codeConnect");
  const codeProps = readRecord(codeConnect, "props");
  const requiredStates = valuesForKeys(variantProperties, ["state", "status"]);

  return ComponentContractSchema.parse({
    figmaNodeId,
    figmaName,
    ...(input.mapping?.codeComponent === undefined
      ? {}
      : { codeComponent: input.mapping.codeComponent }),
    ...(input.mapping?.importPath === undefined ? {} : { importPath: input.mapping.importPath }),
    variantKey: componentVariantKey(figmaName, variantProperties),
    metrics: componentMetrics(input.component),
    propMappings: componentPropMappings({ variantProperties, codeProps }),
    acceptance: {
    ...
    evidenceIds: evidenceForNode(input.evidence, figmaNodeId),
    gapIds: input.mapping?.gapIds ?? [],
  });
}

function componentVariantKey(figmaName: string, variantProperties: Record<string, string>): string {
  const entries = Object.entries(variantProperties).sort(([left], [right]) =>
  ...
  const variantName = figmaName
    .split("/")
    .map((part) => part.trim())
    .filter(Boolean)
    .at(1);

  return variantName === undefined ? "default" : `name=${variantName}`;
}

function componentMetrics(component: Record<string, unknown>) {
  const metrics: Array<{
    name: string;
    value: number | string;
    unit?: string;
    tolerance?: number;
    source?: "figma-node" | "figma-style" | "inferred";
  }> = [];
  const bounds = readRecord(component, "absoluteBoundingBox");
  const fallbackBounds = Object.keys(bounds).length > 0 ? bounds : readRecord(component, "bounds");

  for (const name of ["x", "y", "width", "height"] as const) {
    const value = readNumber(component, name) ?? readNumber(fallbackBounds, name);

    if (value !== undefined) {
      metrics.push({ name, value, unit: "px", tolerance: 1, source: "figma-node" });
    }
  }
  ...
  for (const name of [
    ...
    const value = readNumber(component, name);
  ...
  for (const name of ["shadow", "border"] as const) {
    const value = readString(component, name) ?? readString(readRecord(component, "effects"), name);

    if (value !== undefined) {
      metrics.push({ name, value, unit: "css", tolerance: 0, source: "figma-style" });
    }
  }
...
function componentPropMappings(input: {
  variantProperties: Record<string, string>;
  codeProps: Record<string, unknown>;
}) {
  const codePropEntries = Object.entries(input.codeProps).flatMap(([key, value]) => {
  ...
  return Object.entries(input.variantProperties).map(([figmaProp, figmaValue]) => {
    const sameKey = codePropEntries.find(
      ([codeProp]) => normalizeName(codeProp) === normalizeName(figmaProp),
    );
    const sameValue = codePropEntries.find(([, codeValue]) => codeValue === figmaValue);
    const match = sameKey ?? sameValue;

    return {
      figmaProp,
      figmaValue,
      ...(match === undefined ? {} : { codeProp: match[0], codeValue: match[1] }),
      source: match === undefined ? "missing" : "code-connect",
      required: true,
    };
  });
}
...
function readStringRecord(value: Record<string, unknown>, key: string): Record<string, string> {
  ...
  for (const [recordKey, recordValue] of Object.entries(raw)) {
    ...
    } else if (typeof recordValue === "number" || typeof recordValue === "boolean") {
      result[recordKey] = String(recordValue);
    }
  }
...
function valuesForKeys(record: Record<string, string>, keys: string[]): string[] {
  const normalizedKeys = new Set(keys.map((key) => normalizeName(key)));

  return Object.entries(record)
    .filter(([key]) => normalizedKeys.has(normalizeName(key)))
    .map(([, value]) => value);
}
