...
describe("Design/UI context builder", () => {
  it("writes the required context pack files", async () => {
    ...
    expect(brief).toContain("Normalize Figma style-like component props");
    expect(brief).toContain("Use component contracts as node-level acceptance criteria");

    const implementationPlan = await readFile(context.files.implementationPlanTemplate, "utf8");
    expect(implementationPlan).toContain("variant/token mapping");
    expect(implementationPlan).toContain("component contract metrics");
  });
});
