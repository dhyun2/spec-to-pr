...
describe("decideVisualRepair", () => {
  it("requests another repair attempt when any target scores below 98 percent", () => {
    const decision = decideVisualRepair({
      attempt: 1,
      report: report([
        { targetId: "hero", status: "passed", reviewMatchRatio: 0.99 },
        { targetId: "drawer", status: "passed", reviewMatchRatio: 0.97 },
      ]),
    });
  ...
  it("passes when every compared target meets the visual repair threshold", () => {
    const decision = decideVisualRepair({
      ...
      report: report([
        { targetId: "hero", status: "passed", reviewMatchRatio: 0.99 },
        { targetId: "drawer", status: "passed", reviewMatchRatio: 0.98 },
      ]),
    });
  ...
  it("fails after the maximum repair attempt is exhausted", () => {
    const decision = decideVisualRepair({
      attempt: 3,
      report: report([{ targetId: "hero", status: "failed", reviewMatchRatio: 0.97 }]),
    });
