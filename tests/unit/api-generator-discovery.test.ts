...
describe("discoverApiGenerator", () => {
  ...
  it("prefers frontend API generator packages over fallback generation", async () => {
    await writeFile(
      path.join(directory, "package.json"),
      JSON.stringify({
        devDependencies: {
          "@frontend/api-generator-claude-skill-api-generator": "^0.1.0",
          "api-generator": "^1.1.3",
        },
      }),
    );
    ...
    const plan = await discoverApiGenerator({
      projectRoot: directory,
      sourceKey: "staff",
      targetWorkspace: "apps/rangepro",
    });

    expect(plan).toMatchObject({
      mode: "existing-generator",
      generatorName: "frontend-api-generator",
      generatedRoot: "apps/rangepro/src/shared/api/generated/staff",
      wrapperRoot: "apps/rangepro/src/features",
    });
  });
});
