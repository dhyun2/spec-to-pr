...
describe("Codex SDK workflow policy", () => {
  ...
  it("builds a bounded 98 percent visual repair loop instruction", () => {
    const instructions = buildCodexVisualRepairInstructions({
      minPassingScore: 0.98,
      maxAttempts: 3,
    });

    expect(DEFAULT_CODEX_VISUAL_REPAIR_POLICY.minPassingScore).toBe(0.98);
    expect(instructions).toContain("98.00%");
