...
describe("IntakeRequestService", () => {
  ...
  it("parses Korean file paths without inferring publish intent from generic request words", async () => {
    ...
    const requestText = [
      "./docs/etc/회원관리.pdf랑 docs/기획서.md 읽고 회원관리 화면 구현 요청이야.",
      "project는 apps/rangepro이고 PR은 아직 올리지 마.",
    ].join("\n");
    ...
    expect(result.parsed.filePaths).toEqual(
      expect.arrayContaining(["docs/etc/회원관리.pdf", "docs/기획서.md", "apps/rangepro"]),
    );
    expect(result.parsed.publishPolicy).toMatchObject({
      shouldPublish: false,
      mergeAllowed: false,
