...
describe("QualityGateService", () => {
  ...
  it("records missing gates as skipped checks", async () => {
    ...
    expect(loaded.gaps[0]).toMatchObject({
      title: "Quality gate skipped: lint",
      severity: "major",
    });
  });

  it("runs OpenSpec gate overrides with per-command environment", async () => {
    const result = await service.run({
      runId: "run_11111111111111111111111111111111",
      gates: ["openspec"],
      commands: {
        openspec: {
          command: process.execPath,
          args: ["-e", "process.exit(process.env.OPENSPEC_STRICT === '1' ? 0 : 3)"],
          env: {
            OPENSPEC_STRICT: "1",
          },
        },
  ...
  it("resolves prior quality-gate gaps when a later run passes the same gate", async () => {
    await service.run({
      runId: "run_11111111111111111111111111111111",
      gates: ["build"],
      commands: {
    ...
    expect(loaded.gaps[0]?.resolutionArtifactIds).toContain(result.reportArtifactId);
  });
});
