...
describe("DesignContractService", () => {
  it("generates design contract files, summary, and artifacts", async () => {
    ...
    expect(result.componentContracts).toBe(1);
    ...
    expect(result.changedFiles).toContain(
      "openspec/changes/deliver-reservation-management/artifacts/design-contract/component-contracts.json",
    );
    ...
    expect(summary.componentContracts).toBe(1);
    ...
    expect(
      loaded.artifacts.some(
        (artifact) =>
          artifact.kind === "design-system-map" &&
          artifact.metadata["relativePath"] ===
            "openspec/changes/deliver-reservation-management/artifacts/design-contract/component-contracts.json" &&
          artifact.metadata["componentContractCount"] === 1,
      ),
    ).toBe(true);
  });
});
