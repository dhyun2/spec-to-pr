...
describe("PrReportService", () => {
  it("generates reads and reviews PR report artifacts", async () => {
    ...
    expect(generated.auditMarkdownArtifactId).toMatch(/^art_/);
    ...
    expect(report.markdown).not.toContain("## Figma 제공자 기능");
    ...
    expect(
      loaded.artifacts.some(
        (artifact) =>
          artifact.kind === "pr-report" && artifact.metadata["reportKind"] === "pr-audit-markdown",
      ),
    ).toBe(true);
  });
