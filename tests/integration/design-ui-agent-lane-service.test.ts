...
describe("DesignUiAgentLaneService", () => {
  it("prepares context and records design-ui result", async () => {
    ...
    const designContractContent = await readFile(context.files.designContract, "utf8");

    expect(JSON.parse(designContractContent)).toMatchObject({
      screens: [
        {
...
async function buildRun() {
  ...
  const designContractBlob = await artifactStore.writeBlob({
    content: Buffer.from(
      `${JSON.stringify(
        {
          screens: [
    ...
    artifacts: [
      {
        ...
        uri: designContractBlob.uri,
        mediaType: "application/json",
        digest: designContractBlob.digest,
