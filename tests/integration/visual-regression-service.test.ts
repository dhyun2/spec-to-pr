...
describe("VisualRegressionService", () => {
  ...
  it("records legacy-vs-target PNG comparisons as official visual report JSON", async () => {
    const run = await store.get("run_11111111111111111111111111111111");
    const baseline = run.artifacts.find((artifact) => artifact.kind === "figma-screenshot");
    const target = run.artifacts.find((artifact) => artifact.kind === "screenshot");

    expect(baseline).toBeDefined();
    expect(target).toBeDefined();

    const recorded = await service.recordLegacyComparison({
      runId: "run_11111111111111111111111111111111",
      changeName: "reservation-list",
      stateName: "Reservation list",
      legacyBaselineArtifactId: baseline!.id,
      targetScreenshotArtifactId: target!.id,
      diffArtifactId: target!.id,
      exactMatch: 0.96,
      reviewMatch: 0.975,
      threshold: 0.98,
      decision: "failed",
      notes: ["Legacy screenshot is the approved baseline for this migration."],
    });

    expect(recorded.decision).toBe("failed");
    ...
    const reportArtifact = loaded.artifacts.find(
      (artifact) => artifact.id === recorded.reportArtifactId,
    );
