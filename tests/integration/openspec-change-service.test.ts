...
describe("OpenSpecChangeService", () => {
  ...
  it("stores generated OpenSpec evidence in the artifact store by default", async () => {
    const run = createInitialRun(
      {
        sources: [
          {
      ...
      loaded.artifacts.some(
        (artifact) =>
          ...
          artifact.metadata["artifactPath"] ===
            "openspec/changes/deliver-reservation-management/artifacts/gap-summary.md",
      ),
    ).toBe(true);

    await expect(
      access(path.join(projectRoot, "openspec", "changes", "deliver-reservation-management")),
