...
`brief` and `feature` require brief + Figma + OpenAPI. `legacy` requires a separate `legacyProjectRoot` and running-legacy baseline; its API candidates are derived through explicitly recorded bounded adapters, while optional OpenAPI only enriches them. Optional `legacyNetworkEvidencePath` accepts bounded standard HAR/request JSON up to 1 MB and 1,000 requests and pins its digest plus `runtime-network-har` adapter. Candidate confidence is retained and an ambiguous method/path without a unique scoped match is a durable intake blocker with no downstream action or submission bypass. `figma` requires a digest-bound deterministic mock manifest and fixtures. Only feature adds `targetedFeatureE2E` and `featureVideo`. Intake pins timestamped `sourceProvenance`; contracts declare `visualTargets` and planned legacy coverage; implementation records current-packet legacy/API coverage and `performanceEvidence`. Every visual capture repeats its target route/state/viewport/device scale/fixture and records provider, ISO capture time, PNG path, and digest; runtime rejects target drift or digest mismatch and enforces at least 98% review ratio, no more than 20% masking, and three total comparison attempts. Report emits ready/blocked 15-section `pr-report-v2.1` JSON and Markdown, and new publication verifies the legacy adapter list plus inventory digest.
