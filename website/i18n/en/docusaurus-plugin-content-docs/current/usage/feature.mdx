...
4. **UI / visual / performance:** Connect UI/API, make `apiCoverage` exactly match all intake operations, and record Web Vitals. Every actual capture repeats the target route/state/viewport/device scale/fixture and records provider, ISO capture time, PNG path, and `sha256:` digest. Runtime rejects target drift or digest mismatch before Figma/result `compare-visuals`, which requires at least 98%, at most 20% justified masking, and three total comparison attempts (the initial comparison plus at most two repairs).
...
| Visual fidelity | `evidence/visual/checkout-report.json`            | Figma/result ratio ≥ 98%, baseline/target/diff/overlay                              |
