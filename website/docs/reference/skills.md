...
`implement`는 하나의 구현 맥락을 유지합니다. UI/feature 후보의 브라우저 근거는 `capture-session-v1` 하나로 수집하고, 의존 입력의 `evidence-fingerprint-v1`이 유지될 때만 같은 Run에서 검증된 결과를 재사용합니다. 런타임은 반복 상태 조회나 같은 테스트를 병렬화하지 않습니다. UI 구현 뒤에는 작업량과 관계없이 변경할 수 없는 같은 검토 묶음에서 두 독립 검토자가 함께 판정하며, 결과를 packet inbox에서 함께 적용합니다.

조정·상태·발행은 선택한 모델의 `medium` 추론으로, 구현과 기능·디자인 검토는 같은 모델의 `high` 추론으로 실행합니다. 이 라우팅은 기존 구현 맥락을 끊거나 검토자의 읽기 전용 경계를 바꾸지 않습니다.
