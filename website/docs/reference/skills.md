...
`legacy-source-inventory.cjs`는 정확한 레거시 source path를 유지하면서 local import graph를 읽기 전용으로 따라가 supporting dependency, route 선언·navigation, asset URL, CSS selector·breakpoint, 지도/Swiper/native bridge 표식을 추출합니다. `legacy-visual-evidence.cjs`는 이 inventory mapping과 모든 사용자 노출 route/state, 실제 fixture·최종 URL·핵심 UI·필요 API/인증·관련 오류, Vue 3 대상 코드 규격을 함께 검증합니다. 기준·이관·Diff 이미지가 Git index에 있는지도 확인하고, PR 상단 Gap과 라우트 동작 요약 뒤에 경로·상태별 좌우 이미지와 Diff 링크를 만듭니다. legacy 캡처는 Computer Use 우선이며, Browser/Playwright fallback은 provider·인증 상태·사유를 한 번 공개하되 같은 조건의 증빙을 확보했다면 Gap으로 만들지 않습니다. 보존 이관에서 `@frontend/ui`, Unicode glyph/emoji, mock/placeholder 대체가 발견되거나 화면이 비어 있고 API/CORS 오류가 있으면 `NOT VERIFIED` Gap으로 표시합니다.
