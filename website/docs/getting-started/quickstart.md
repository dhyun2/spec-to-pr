...
코드 변경은 독립 `functional-reviewer`가 계약, 변경 내역, 관련 테스트, 필수 검증을 검토합니다. UI 변경이면 독립 `design-reviewer`가 시각적 일치도, 상호작용, 디자인 시스템 사용, 접근성을 별도로 검토합니다. 오케스트레이터가 먼저 `workflow_status` 상태와 계약·변경 내역·검증 자료 경로를 고정해 전달하고, 검토자는 workflow 도구를 직접 호출하지 않고 판정 결과만 반환합니다.
