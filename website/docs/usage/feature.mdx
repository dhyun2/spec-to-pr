...
| `report`            | exact selector, structured result, video 하나, reviewer verdict와 risk를 canonical report로 만듭니다.                            |
...
Agent 정책은 XS/S read-only scout 0명, M 최대 1명, L/XL 최대 2명입니다. Scout는 source discovery만 읽고, API와 UI는 한 implementation writer가 함께 편집합니다. **no nesting**, **parallel writer 금지**이며 functional/design reviewer 병렬 실행은 implementation 뒤에만 허용됩니다.
