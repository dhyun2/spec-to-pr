...
- UI 범위면 mode와 관계없이 모든 route/state/viewport 비교 대상을 실행하고 결과를 남깁니다.
- API·binding·인증·증빙 분석 실패는 확인된 구현을 멈추지 않습니다. 안전하지 않은 쓰기만 중단하고, 나머지는 open Gap으로 남깁니다.
- `skipped`와 `waived`는 `passed`가 아니며, Draft는 Gap·실패·미실행 증빙과 함께 만들어질 수 있어도 verified/merge-ready가 될 수 없습니다.
- 사용자가 준 `legacyProjectRoot`와 `targetPaths`는 유사 경로 추론 없이 그대로 사용합니다. OpenSpec은 선택적 사후 adapter이며 core Run의 전제 조건이 아닙니다.
...
| `implementation`    | 확인된 동작부터 구현합니다. 불명확한 API·binding은 Gap으로 남기며, UI는 화면 비교를 반드시 실행하고 `feature`는 영상 1개도 필요 |
| `functional-review` | 코드 범위에 필요한 기능 검증과 요구사항을 독립 검토자가 승인                                                                    |
| `design-review`     | UI 범위의 화면·상호작용·접근성 근거를 독립 검토자가 승인. 비-UI 작업에는 적용하지 않음                                          |
| `report`            | 현재 packet의 기계용 보고서와 네 가지 reviewer-first PR 본문 생성                                                               |
| `publish`           | `publication: draft`일 때 초안 PR/MR과 필수 첨부 자료 동기화                                                                    |
| `archive`           | 신뢰할 수 있는 병합 근거가 있을 때 사용자가 명시적으로 실행                                                                     |

각 단계의 `pending`, `running`, `passed`, `failed`, `blocked`, `skipped`, `waived` 상태와 실행 권한, 체크포인트는 영구 기록에 보관됩니다. `skipped`와 `waived`는 통과가 아닙니다. 사용자는 내부 상태 전이용 세부 도구 대신 `workflow_advance`와 `workflow_status`를 사용합니다.
...
API와 UI를 별도 에이전트나 작업 트리로 나누지 않으므로 맥락을 넘기거나 나중에 합치는 단계가 없습니다. 확인된 API는 같은 구현 맥락에서 타입·호출 모듈·테스트로 검증합니다. 그러나 메서드·body·인증 경계가 불명확하면 요청을 추측해 쓰지 않고 해당 상호작용을 안전하게 비활성화한 뒤 Gap으로 공개합니다. 이 분석 실패가 이미 확인된 UI·상태·라우트 구현을 막지는 않습니다.

## 브라우저 근거는 한 번에 수집합니다

UI 또는 `feature` 후보는 필요한 브라우저 근거를 후보별 `capture-session-v1` 하나에 묶습니다. 이 세션은 변경 기능 E2E, 정확히 하나의 feature 영상, 화면 비교용 실제 캡처, 필요한 성능 결과를 **최대 한 번의** Playwright Test/CLI 호출에서 수집합니다. 매니페스트에는 후보의 base/head/diff 해시, 선택자, 실행 환경, 결과 파일 해시를 남기므로 이후 `compare-visuals`는 이미 수집한 실제 캡처를 현재 검토 묶음용 영수증으로만 연결하며 브라우저를 다시 실행하지 않습니다.

같은 Run 안에서 의존 입력의 해시가 변하지 않은 근거는 `evidence-fingerprint-v1`로 안전하게 이어 쓸 수 있습니다. 선택자·도구 버전·fixture·import closure·결과 파일 해시까지 일치해야 하며, 요구사항이나 코드 차이가 바뀌면 새 검토 묶음은 항상 만듭니다. 재사용은 통과 조건을 줄이거나 다른 Run의 오래된 근거를 복사하는 기능이 아닙니다.

요청을 접수할 때 로컬·원격 자료의 원본 해시, 조회 시각, 확인된 위치를 `sourceProvenance`에 고정하고 OpenAPI의 확인된 작업 목록을 만듭니다. UI 작업은 mode와 무관하게 Figma나 실행 중인 레거시의 기준 화면을 `visualTargets`로 고정합니다. 실제 캡처에는 기준과 같은 경로·상태·화면 크기·기기 배율·테스트 데이터, 자료 제공자, ISO 형식의 촬영 시각, PNG 경로와 `sha256:` 해시를 제출합니다. `compare-visuals`는 비교 조건이 달라지거나 해시가 맞지 않으면 시도 횟수를 소비하지 않고 거부하며, 정확 일치율·검토 일치율·차이 이미지·겹침 이미지를 직접 계산합니다. 호출자가 미리 계산한 점수는 받지 않습니다. 고정 기준은 일치율 92% 이상, 근거가 있는 마스킹 영역 20% 이하입니다. 최초 비교와 보정 후 최대 2회를 사용자 확인 없이 자동으로 수행합니다. 세 번째 유효 실패는 자동 보정만 멈추고 동일 크기 기준/결과와 차이·겹침 이미지를 **화면 비교 Gap**으로 남깁니다. 집중 UI assertion은 전체 점수와 별도로 판정하며 `skipped`·`waived`·`not-run`은 통과가 아닙니다.

레거시 계약은 안정적인 기능 키를 `planned`로 고정하고, 최종 구현에서 현재 검토 묶음의 상태를 `migrated` 또는 범위 제외로 바꿉니다. `legacy`는 사용자가 준 `legacyProjectRoot`와 `targetPaths`만 정확히 읽고 이름이 비슷한 경로를 추론하지 않습니다. 메서드·경로·body·인증 경계가 모호한 API·binding·증빙 분석은 안전하지 않은 쓰기를 하지 않는 한 구현을 멈추지 않고 **open Gap**으로 남깁니다. 확인된 화면·상태·라우트와 안전한 읽기 작업은 계속 구현하며, HAR·요청 JSON·OpenAPI는 같은 Run에서 Gap을 해소하는 선택 증거입니다.
...
모호한 레거시 API의 범위 제한 HAR·요청 JSON 또는 OpenAPI는 Gap을 해소할 때만 제출합니다. 실행 자료의 해시와 탐지 방식은 인벤토리에 고정되지만, OpenSpec이나 추가 증빙을 사용자가 미리 준비해야 core Run이 시작되는 것은 아닙니다.
...
구현이 끝나면 워크플로 조정자가 `workflow_status` 상태 기록, 승인된 계약, 코드 차이, 증거 경로를 변경할 수 없는 검토 묶음으로 만들어 두 검토자에게 전달합니다. UI 작업은 기능·디자인 검토를 같은 묶음에서 즉시 동시에 시작합니다. 결과는 먼저 묶음별 inbox에 보관하고 적용 대상 판정이 모두 도착한 뒤 한 번에 다음 상태를 정합니다. 수정 요청이 하나라도 있으면 두 판정과 함께 구현을 한 번만 다시 열며, 한 검토자가 다른 검토자의 판정을 대신하거나 두 결과를 하나의 통합 점수로 합치지는 않습니다.
...
실행 시간을 줄이기 위해 일반적인 보조 작업, 반복 상태 조회, 같은 검증의 재실행을 병렬화하지 않습니다. 독립적이고 범위가 겹치지 않는 읽기 전용 조사만 예상 작업량에 따라 위 표의 한도에서 병렬화하며, UI 구현 후의 두 독립 검토는 작업량과 관계없이 같은 고정 검토 묶음에서 병렬로 실행합니다. 검토자 시간 초과는 대체 검토자를 자동으로 만들지 않는 진단 상태입니다.

조정·상태·발행 같은 기계적인 경계는 선택한 실행 모델의 `medium` 추론으로, 구현과 기능·디자인 검토 경계는 같은 모델의 `high` 추론으로 이어집니다. 모델이나 대화를 새로 쪼개지 않고 경계마다 필요한 추론 수준만 바꿉니다.
...
기계용 `pr-report-v2.1`은 증거를 보존하지만, PR 본문은 목적별 네 템플릿 중 하나만 사용합니다. Legacy migration은 원본→대상·이관 범위·레거시 화면 비교·API Gap, Brief delivery는 요구사항 충족표·제외 범위·화면 비교, Feature flow는 전후 동작·회귀 검증·필수 사용자 흐름 영상·화면 비교, Figma UI는 상태 매핑·상태별 일치율·디자인·접근성 검증을 보여 줍니다. 모든 Gap은 상단에서 영향과 리뷰어 결정을 함께 표시하고, 내부 로그·Run ID·빈 체크리스트는 기본 본문에서 제외합니다.
...
레거시 이관의 core 검토 자료는 `.spec-to-pr/<feature>/`에 보존합니다. OpenSpec은 병합 뒤의 선택적 변경 기록이며, core Run이나 Draft PR의 전제 조건이 아닙니다. 원본 HAR·쿠키·토큰·전체 로그, 내부 Run ID, 불투명한 기능 키, 반복되는 전체 구현 파일 목록은 검토 본문에서 제외합니다.
