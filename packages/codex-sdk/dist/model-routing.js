...
export function resolveCodexModelRouting(input, legacyPinnedModel) {
    const strategy = input?.strategy ?? (legacyPinnedModel === undefined ? "adaptive-verified" : "pinned");
    ...
    const requested = strategy === "pinned"
    ...
    const qualityGaps = [];
    if (strategy !== "pinned") {
        for (const role of ["expert", "build"]) {
            if (!unavailable.has(role))
                continue;
            const fallbackRole = role === "expert" && !unavailable.has("build") ? "build" : "fast";
            if (unavailable.has(fallbackRole))
                continue;
            ...
            qualityGaps.push({
                ...
                reason: "The requested higher-capability model was unavailable; the host used the next available configured role.",
            });
        }
    }
    return {
...
function resolveCustomModels(models) {
    if (models?.fast === undefined ||
