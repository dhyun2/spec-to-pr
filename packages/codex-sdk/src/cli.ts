...
type ParsedArgs = {
  ...
  resume?: string;
  model?: string;
  modelRouting?: "adaptive-verified" | "pinned" | "custom";
  pinnedModel?: string;
  fastModel?: string;
  buildModel?: string;
  expertModel?: string;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--model-routing":
        ...
        parsed.modelRouting = value as NonNullable<ParsedArgs["modelRouting"]>;
        break;
      case "--pinned-model":
...
function secondsToMilliseconds(seconds: number, argument: string): number {
