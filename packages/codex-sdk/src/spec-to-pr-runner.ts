...
function modeInstructions(mode: NonNullable<SpecToPrCodexRunInput["deliveryMode"]>): string {
  ...
  if (mode === "legacy") {
    return "Legacy mode is cross-project migration: require a separate legacyProjectRoot, treat it as read-only, inspect workflow_status.legacyInventory, map every in-scope stable feature key through legacyCoverage, derive API candidates from the explicitly listed bounded source adapters, run both projects, use the running legacy screen as the mandatory visual baseline, compare the target at the same route/state/viewport through compare-visuals, and report migration/API/performance gaps in pr-report-v2.1. Treat legacyProjectRoot and workflow_status.legacyInventory as the complete source boundary. Resolve every request term only against that inventory; if no feature key matches, report an in-bound scope mismatch. A sibling, parent, or keyword-similar module requires an explicit replacement legacyProjectRoot and is never inferred from repository-wide search. When source discovery leaves an ambiguous method or path, optionally supply a project-local legacyNetworkEvidencePath containing bounded HAR JSON or a request array captured from the scoped legacy flow. API candidates require complete api-ready/apiCoverage evidence; zero candidates require a complete API section bound to the adapter list and inventory digest. Ambiguous methods/paths resolve only from a unique scoped OpenAPI/runtime match and otherwise remain a durable intake blocker. Optional OpenAPI enriches candidates but never controls API-section applicability.";
  }
  if (mode === "feature") {
    return [
