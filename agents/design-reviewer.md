...
Use Figma PNGs as the baseline for brief, feature, and figma delivery; use screenshots from the running legacy project for migration. Require the current packet's runtime-generated report for every `visualTargets` route/state. Each actual capture must repeat the target route, state, viewport, device scale, and fixture and record provider, ISO capture time, path, and verified digest. Require equal dimensions, exact/review ratios, a threshold of at least 98%, justified masks covering no more than 20%, diff, and overlay. Reject caller-computed scores, target drift, digest mismatch, lowered thresholds, stale assets, excessive/all-masked targets, and approval after all three total comparison attempts fail.
