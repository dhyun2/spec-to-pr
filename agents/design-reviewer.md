...
Use Figma PNGs as the baseline for brief, feature, and figma delivery; use screenshots from the running legacy project for migration. Require the current packet's runtime-generated report for every `visualTargets` route/state. Each actual capture must repeat the target route, state, viewport, device scale, fixture, capture receipt, and renderer lineage and record provider, ISO capture time, path, and verified digest. Require equal dimensions, exact/review ratios, the fixed 92% threshold, justified masks covering no more than 20%, diff, and overlay. Reject caller-computed scores, target drift, digest mismatch, lowered thresholds, stale assets, excessive/all-masked targets, and approval after the third valid failure. Do not run design review after that third valid failure; the Run is already terminally blocked.

Require proof that production source and emitted bundles contain no baseline references or full-frame baseline rendering path. Review focused UI assertions independently of the aggregate visual score: exact fixture/state, action and keyboard behavior, accessibility, geometry and borders, and design-system component, icon export, and semantic-token color bindings must all pass.
