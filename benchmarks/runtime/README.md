...
The benchmark uses one warm-up and five measured iterations for deterministic mixed-intake, legacy-discovery, compact-status, mutating-action, packet-evidence, and visual-comparison fixtures. The packet-evidence fixture runs a real strict-workspace implementation through functional review and report generation, then derives freshness reuse and binary-diff recapture counts from the recorded Git metrics.
