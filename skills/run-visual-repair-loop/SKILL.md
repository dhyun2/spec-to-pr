...
argument-hint: "<run-id> <change-name> <base-url> [min-score=0.98] [max-attempts=3]"
...
```text
<run-id> <change-name> <base-url> [min-score=0.98] [max-attempts=3]
```
...
Component contracts use their own component-level threshold. When a target has
`comparisonScope: "component"`, do not replace that gate with a full-screen
score; repair the component crop/state until the component visual report passes
or the loop fails with explicit blocker evidence.
