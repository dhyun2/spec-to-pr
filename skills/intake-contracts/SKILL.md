...
- When `deliveryProfile.draftEvidenceBundle` is explicitly present, treat it as an optional compatibility evidence location. Keep production code and test source in the target project's normal feature locations; write only concise final review evidence under the supplied roots. Never put a run ID in a directory name. Do not create `openspec/` paths, a bundle, or any OpenSpec manifest merely to start or pass the core Run; OpenSpec is a post-merge optional adapter. Do not copy raw browser logs, credentials, headers, full HAR files, or intermediate captures into an optional bundle.
...
- Legacy OpenAPI is optional enrichment, not an applicability switch. Derive API candidates from the bounded inventory. Preserve the zero-candidate inventory digest so the final API section can be `complete` with no operations. An unresolved method, dynamic binding, auth requirement, or body shape is an open API Gap: implement confirmed routes/UI/read behavior, leave unsafe writes disconnected or safely stubbed, and ask for the reviewer decision rather than blocking contracts.
- For any supplied `figmaUrl`/`figmaUrls`, use the host's connected Figma capability to fetch real nodes, screenshots, variables, assets, and component context for every normalized URL. Before contracts, submit exactly one `kind: figma-bundle` with `provider: host-connected-figma`, ISO `capturedAt`, compatibility `fileUrl`, complete `fileUrls`, nonempty `nodeIds`, captured component references, a strict `designMapping`, a declared JSON `manifestPath`, and project-local `artifactPaths` containing that manifest plus one or more actual PNG files. The manifest repeats that provenance and exactly lists its PNG `visualPaths`. Require native `figma-capture-geometry-v2` for every new target: node/state, capture kind, logical frame size, export scale, decoded bitmap size, and sRGB color space must agree. A v1 record or downscaled host thumbnail is display-only evidence and must be reacquired before contracts.
- Enumerate captured state facts before contracts and bind each target/node/state to its own stable fixture and state-contract digest; distinct captured states may not share a fixture when any fact differs. Honor an explicit design-system package such as `@lessonpro/ui` or `@frontend/ui`, then map every captured component and icon through its verified package manifest, exact package version, and public barrel export. Code Connect is optional: do not require, create, or query a mapping when the user requests Figma MCP only or repository evidence does not use it. When user input and repository evidence resolve exactly one package and Code Connect policy, proceed without a question; otherwise ask one concise question before constructing the Figma bundle. Record the exact export and props, semantic token, and expected geometry including width, height, alignment, and flex shrink. Known internal icons such as Spot, Circle, and Close require their exact public exports and icon-role tokens; an external or merely similar substitute is not an exception.
...
Submit `contracts` with `passed`, `failed`, or `blocked`, compact artifact paths, and a nonempty `requirementManifest` when passed. Each requirement has a stable ID, title, and concrete acceptance criteria. Include `guidanceTrace`, structured `legacyBaseline` and planned `legacyCoverage` when available, and scope each legacy row with `legacyScopeKeys`. Preserve the intake `sourceProvenance` and authoritative OpenAPI operation inventory; do not replace them with human claims. When the change surface is known, submit numeric `workloadSignals` with at least one observed field besides uncertainty. Use `blocked` only when a safe write target or a required source for the selected delivery cannot be identified; a missing legacy baseline, API match, auth, or optional evidence source is `not-run + Gap`, not a reason to stop confirmed implementation. Advance only after the accepted submission and refined workload are visible in `workflow_status`.
