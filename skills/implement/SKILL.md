...
2. Follow the accepted requirements and `guidanceTrace` in precedence order. Apply requested `skillHints` only when available and applicable: use `api-generator` for OpenAPI work, `design-system` for mapped UI components, and `react-best-practices` or `next-best-practices` for their respective implementation concerns. Project guidance wins over generic skill advice; a missing optional skill never blocks implementation.
