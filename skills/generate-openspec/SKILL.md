...
By default, generated OpenSpec markdown/json is stored in the Run artifact store,
not written into the target repository. Only pass `writeToProject: true` when the
user explicitly asks to export OpenSpec files into the project tree.
...
3. Report generated artifact IDs and changed files. In the default artifact-store mode,
   `changedFiles` should be empty.
