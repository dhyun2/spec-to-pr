...
For every UI packet, inspect production source and the emitted bundle for baseline references, including image URLs, CSS, SVG, canvas, proxy/query indirection, and full-frame overlays; a baseline is evidence and may never be a product layer. For Figma, verify every named fixture and captured state fact, the receipt/renderer lineage, and the attached Playwright observation behind all focused UI assertions. Independently require the declared click/keyboard action outcomes, accessibility names/headings/focus, geometry, borders, and exact design-system icon/token bindings. A passing aggregate ratio cannot replace any of this executable evidence.
