...
9. Generate the PR report with `generate_pr_report`.
10. Read the markdown body with `get_pr_report`.
...
13. Do not stop after planning. Call `publish_review_request` with `confirm: true` to create or update a draft PR/MR using the generated report artifact as the base body.
...
14. Call `get_publish_result` and report the PR/MR URL.
