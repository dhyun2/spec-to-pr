...
6. `legacy`는 **화면 계약은 보존하고 구현 방식은 대상 저장소 규격으로 이관**합니다. Vue 3가 실행된다는 이유만으로 Options API·mixin·Vuex 호환층을 완료로 보지 않습니다.
...
3. 제공 자료와 대상 프로젝트의 기존 구조를 읽습니다. `legacy`는 `AGENTS.md`, package 설정, 인접 표준 페이지에서 Vue SFC·TypeScript·상태 관리·router·호환 계층 규칙을 확인해 `targetCodeProfile`을 정합니다. `brief`·`feature`의 OpenSpec은 에이전트가 만들 수 있는 보조 산출물이며, 없다는 이유로 core 구현을 멈추지 않습니다.
4. UI를 구현합니다.
   - `brief`, `feature`, `figma`: 대상 프로젝트의 디자인 시스템과 기존 컴포넌트를 우선 사용합니다.
   - `legacy`: 기본 전략은 **보존 이관**입니다. 레거시 템플릿·클래스·CSS·자산·스프라이트·컨트롤·사용자 동작은 유지하되, script·state·router·utility는 `targetCodeProfile`에 맞게 변환합니다. 대상 규격이 `<script setup lang="ts">`·Pinia·Vue Router 4라면 Options API·mixin·Vuex 호환층·Vue 2 Event Bus를 복사해 남기지 않습니다. `@frontend/ui` 등 디자인 시스템으로 화면을 대체하거나 UI를 재구성하지 않습니다. Unicode glyph/emoji, 문자·CSS로 그린 로고·핀·아이콘, 지도·carousel의 placeholder/mock 대체도 금지합니다. 사용자가 명시적으로 재디자인을 승인한 경우만 승인 문구를 증빙에 기록합니다.
...
6. `legacy`는 화면 캡처 전에 source inventory를 읽기 전용으로 생성합니다. 이 inventory는 정확한 `sourcePaths`를 이관 범위로 유지하면서 상대 경로·`@/` 로컬 import를 따라 supporting dependency를 별도로 기록하고, router 선언·실제 navigation, 이미지·sprite·font URL, CSS selector, media breakpoint, 지도 SDK·carousel·native bridge 표식을 뽑습니다.
   ...
   manifest는 이관 범위에서 발견한 모든 route를 화면 매트릭스에, 모든 asset·selector·breakpoint·runtime 표식을 1:1 mapping에 넣어야 합니다. supporting dependency는 읽기 증거일 뿐 `targetPaths`를 자동 확장하지 않습니다. 해석하지 못한 동적 import·navigation은 개발을 멈추지 않고 Gap으로 남깁니다. 원본 asset은 대상 파일 또는 canonical URL로 매핑하고, selector와 breakpoint는 대상 CSS에 실제로 존재해야 합니다. 실제 Kakao Map·Swiper·bridge를 회색 박스나 CSS로 대체하면 안 됩니다.
...
7. 화면을 캡처하고 비교합니다.
   ...
   - 무거운 전체 E2E나 영상은 요구하지 않습니다. 대신 화면별 `routeCheck`에 레거시 UI/read API에서 얻은 실제 fixture와 동적 파라미터, 최종 URL, 구체적인 selector/text 기대값, API 필요 여부와 필요한 API 결과, 인증 증거, console/network 진단 실행 여부와 관련 오류를 기록합니다. 여러 경로가 있으면 대표 클릭·선택 전환을 한 건 이상 포함하고, 위험한 write 확정 직전까지만 확인해도 됩니다.
   - `0`, `test`, `today` 같은 임의 동적 파라미터, 직접 URL만 연 클릭 경로, 인증 문자열만 있고 성공 증거가 없는 캡처, loading·예상 밖 빈 화면·오류 화면, 필수 API 비-2xx/CORS, 관련 console/network 오류는 픽셀이 같아도 통과가 아닙니다.
   ...
   - 빈 영역이 점수를 부풀리지 않도록 legacy target마다 검색·필터·목록·지도 컨트롤 같은 핵심 UI 영역을 하나 이상 지정해 전체 화면과 별도로 비교합니다. 전체 viewport 하나를 `criticalRegions`로 재사용하지 않습니다.
8. `legacy`는 `spec-to-pr-evidence/<change>/legacy-visual-manifest.json` schema v4를 만들고 아래 도구로 검증합니다. 이 manifest는 Computer Use 우선 capture policy, baseline·actual provider/인증 상태/fallback 사유, `routeChecks`, `targetCodeProfile`을 포함합니다. 도구는 빠진 화면, 런타임 화면 유효성, source asset/CSS/runtime mapping, Vue 3 대상 규격, 금지된 glyph·placeholder, 비교 이미지의 Git index 누락을 숨기지 않고 `NOT VERIFIED`와 Gap을 만듭니다. v2/v3 manifest도 읽지만 새 의미 증빙이 없으므로 검증 완료로 승격하지 않습니다. Computer Use 미노출·캡처 불가 사유가 기록된 Browser/Playwright fallback은 캡처 방법으로만 공개하며 Gap으로 만들지 않습니다.
...
9. 구현과 증빙을 읽지 않은 새 컨텍스트에서 기능 검토하고, UI가 있으면 디자인·접근성 검토도 합니다. `legacy`는 UI 보존과 Vue 3 대상 코드 규격을 별도로 확인합니다. `passed` 판정만 검증 통과로 씁니다. 실패·미실행·모델/환경 부재는 Gap이며, 상위 모델을 쓸 수 없다고 이 검토를 생략하거나 통과로 바꾸지 않습니다.
10. 최종 `git diff`, 화면 증빙, 검증 결과를 바탕으로 **case 전용** PR 템플릿 하나를 채웁니다. 템플릿 선택 기준은 [assets/pr-templates/README.md](assets/pr-templates/README.md)입니다. 중요한 Gap은 검토자 결정 바로 아래에 놓고, route 동작·좌우 이미지·Vue 3 규격을 간결하게 표시합니다. 내부 실행 식별자·원시 로그, 토큰·쿠키, 빈 표·체크리스트, 중복 discovery 행은 PR 본문에 넣지 않습니다.
