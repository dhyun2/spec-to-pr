...
5. Perform that action with the matching skill, then record its compact evidence with `workflow_submit`. For review actions, the orchestrator first calls `workflow_status`, freezes a review packet containing that snapshot, accepted contracts, diff, and evidence paths, and passes the packet to the applicable independent reviewer. Reviewers do not call workflow tools; they return literal schema-shaped submission objects for the orchestrator to validate and submit.
