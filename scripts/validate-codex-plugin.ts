...
const codexManifestVersionPattern = new RegExp(
  `^${escapeRegex(packageJson.version)}(?:\\+codex\\.[0-9A-Za-z.-]+)?$`,
);

const CodexPluginManifestSchema = z
  .object({
    name: z.literal("spec-to-pr"),
    version: z.string().regex(codexManifestVersionPattern),
...
const mcpArgs = manifest.mcpServers.spec_to_pr.args;
